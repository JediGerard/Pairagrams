...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (const [word, entry] of Object.entries(masterWords)) {
    if (!entry || word.length !== 6) continue; // Changed to 6

    const isCommon = entry.common === true;
    // For 6-letter words, common4 might need a different name or logic if it's specific to 4-letter words.
    // Assuming for now it's okay to add 6-letter common words to it, or this set is less relevant here.
    if (isCommon) common4.add(word.toUpperCase());

    const cleanSplit = entry.parts.find(p =>
      Array.isArray(p) &&
      p.length === 2 && // Still 2 parts (e.g., left and right)
      p[0].length === 3 && // Each part is 3 letters
      p[1].length === 3  // Each part is 3 letters
    );
...
window.onLetterSelected = function(letter) {
  ...
  // const totalChars = previewLetters.join("").length; // Not needed, using previewLetters.length
  if (previewLetters.length >= 2) // Max 2 pairs (each pair is a 3-letter string)
    {
    ...
    if (!soundOff && typeof showFeedbackMessage === "function") {
      ...
      showFeedbackMessage("Max 2 pairs");
    } else {
  ...
  previewLetters.push(letter); // letter is a 3-char string (a pair)
...
window.submitWord = function() {
  ...
  if (word.length < 6) { // Check combined length of pairs
